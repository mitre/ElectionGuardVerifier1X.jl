...
function verify_proofs(c::Constants, ctx::Context, g::Guardian)::Int64
    bitor(p -> verify_schnorr(c, ctx, p), g.coefficient_proofs)
end
...
"Verify that K_ij is in Z^r_p (Item D)."
function verify_schnorr_d(c::Constants, p::Schnorr_proof)::Int64
    within_mod(p.public_key, c.q, c.p) ? 0 : D
end
